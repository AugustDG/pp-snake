...
- `.hpp` is more explicit, indicating a C++specific header file.
...
Clarify the conventions between `.h` and `.hpp` files
...
- Default Initialization
    - For fundamental types like `int`, if you don't initialize it, the value of `x` will be **indeterminate** (essentially garbage data). In practice, this means that `x` could contain any random value.
...
- Direct Initialization
    - For fundamental types like `int`, value initialization will **set the value to zero**. In this case, `y` is guaranteed to be initialized to `0`.
    - The expression `new int{}` allocates memory for an integer and initializes it to **0**. So, the memory that `y2` point


## Struct vs. Class
Struct vs. Class can often be used interchangeably in C++, but there are key differences in their typical use cases.
...
### Preventing Implicit Conversion
 To prevent implicit conversions, you can use the `explicit` keyword. This forces the programmer to use **explicit** conversions, making the code clearer and reducing the chance of errors.
...
Instead, you'll need to explicitly create the `Person`
```cpp
Person d = Person(3); // Explicit conversion, this is allowed
...
Would you rather catch errors during **compile time**, **link time**, or **runtime**? Ideally, you want to catch bugs as early as possible—at **compile time**—to make your code more reliable and easier to maintain.



**If something won’t change, mark it as `const`.**
...
```
This lack of protection can lead to unintended side effects, especially when passing objects by reference:
...
Now, the caller knows that the function will **not modify** the passed object.
...
The `constexpr` keyword allows you to compute values at compile time.
...
Just cause you know how to use pointers, doesn’t mean you know how to use ptrs
